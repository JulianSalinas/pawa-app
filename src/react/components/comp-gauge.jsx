        ...
        return (
            ...
            <svg height="100%" version="1.1" width="100%" xmlns="http://www.w3.org/2000/svg" style={{
                ...
                <path fill={this.props.backgroundColor}
                      stroke="none" d={this._getPath(this.props.max)}
                      filter={"url(#" + this.uniqueFilterId + ")"}/>

                <path fill={this.props.color}
                      stroke="none" d={this._getPath(this.props.value)}
                      filter={"url(#" + this.uniqueFilterId + ")"}/>

                <text x={this.props.width / 2}
                      y={this.props.height / 8}
                      textAnchor="middle"
                      style={topLabelStyle}>{this.props.label}
                </text>

                <text x={this.props.width / 2}
                      y={this.props.height / 5 * 4}
                      textAnchor="middle"
                      style={valueLabelStyle}>{ this.props.valueFormatter(this.props.value) }
                </text>

                <text x={((Cx - Ro) + (Cx - Ri)) / 2} y={Cy + 25}
                      textAnchor="middle"
                      style={this.props.minMaxLabelStyle}>{this.props.min}
                </text>

                <text x={(Xo + Xi) / 2}
                      y={Cy + 25}
                      textAnchor="middle"
                      style={this.props.minMaxLabelStyle}>{this.props.max}
                </text>
